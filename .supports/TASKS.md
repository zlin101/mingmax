...
- 目标：吸收 Branch 15 已确认的 `iztro-py` 原生字段，升级内部 chart schema、`chart_facts`、Prompt 和 validator，开放本命盘 + 大限区间级辅助分析。
- 关键交付：`Star.scope`、`ChartMetadata`、`DecadalRange`、`Palace.decadal`、`current_age/current_decadal`、metadata/decadal evidence id、Prompt 大限区间分析边界、validator decadal/metadata evidence 校验。
- 重要决策：信息应尽可能完整进入内部结构和 prompt facts；安全边界由 evidence、supported/unsupported layers 和 validator 管控，而不是通过削弱事实输入来实现。
- 测试摘要：Codex 最终验收时 `uv run black --check .`、`uv run isort --check-only .`、`uv run flake8 .`、`uv run pytest -q` 均通过，测试为 `292 passed`。
- 遗留风险：前端仍是 Branch 12 的轻量核验视图，尚未充分展示 metadata、大限、星曜 scope、当前大限和结构化 LLM evidence；这是 Branch 18 的主要动机。

### Branch 18: 桌面端高信息密度命盘工作台重构

- 分支名：`feature/v0.1-desktop-chart-workbench-ui`
- 目标：将前端从 Branch 12 的轻量核验视图升级为桌面端紫微命盘工作台，充分展示 Branch 16/17 新增的 metadata、大限、星曜 scope 和结构化 LLM 分析。
- 关键交付（HTML）：移除 `chart-verification` 包裹，新增 `chart-workbench`（盘面 + 详情并排），`analysis-summary` 拆分为 `analysis-overview`/`strong-signals`/`weak-hypotheses`/`cross-checks` 结构化容器，`report-section` 改为 `details` 折叠元素。
- 关键交付（JS）：metadata/decadal 渲染至摘要条和中央格，`is-current-decadal` 高亮当前大限宫，宫位大限区间展示，星曜 scope 在详情面板展示，结构化分析渲染替代 `JSON.stringify`，evidence 标签渲染追问问题，所有 `innerHTML` 替换为 `textContent`/DOM 构建。
- 关键交付（CSS）：`max-width: 1440px`，`chart-workbench` flexbox 双栏布局，`palace-detail` sticky 侧边栏，`current-decadal` 克制高亮，分析区结构化样式，移除移动端媒体查询。
- 重要决策：桌面优先，不考虑移动端；继续使用原生 HTML/CSS/JS，不引入前端框架或构建链；所有动态内容 XSS 安全（仅 `textContent`/DOM text node）。
- 测试摘要：新增 8 个前端测试（共 33 个前端测试），全量 `uv run pytest -q` 通过 `300 passed`。
- 遗留风险：不做移动端适配；前端仍为静态单页，无用户系统、无保存历史；流年/流月/流日/流时 UI 未实现。
