...
from app.llm.openai_compatible import LLMClientConfigError, OpenAICompatibleLLMClient


def _clear_llm_env(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    monkeypatch.delenv("MINGMAX_LLM_WIRE_API", raising=False)
    monkeypatch.delenv("MINGMAX_LLM_TIMEOUT_SECONDS", raising=False)


def test_provider_mock_returns_mock_client(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
...
def test_provider_openai_compatible_returns_real_client(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    _clear_llm_env(monkeypatch, tmp_path)
    ...
    monkeypatch.setenv("MINGMAX_LLM_WIRE_API", "responses")
    monkeypatch.setenv("MINGMAX_LLM_TIMEOUT_SECONDS", "15")

    client = _get_llm_client()
    assert isinstance(client, OpenAICompatibleLLMClient)
    assert client._model == "gpt-4"
    assert client._wire_api == "responses"
    assert client._timeout == 15


def test_unknown_provider_raises_config_error(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    _clear_llm_env(monkeypatch, tmp_path)
    monkeypatch.setenv("MINGMAX_LLM_PROVIDER", "typo")

    with pytest.raises(LLMClientConfigError, match="Unsupported LLM provider"):
        _get_llm_client()
